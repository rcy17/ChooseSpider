...
def get_comment(session: Session):
    ...
    r = session.post('https://webvpn.tsinghua.edu.cn/http/77726476706e69737468656265737421eaff4b8b3f3b2653770bc7b'
                     '88b5c2d320506b1aec738590a49ba/xkBks.xgpg_xspjyxkt.do?vpn-12-o1-zhjwxk.cic.tsinghua.edu.cn&'
                     'cm=xgpg_qbkcmycdzbData&p_xnxq=2020-2021-1&p_xslb=bks', data=data)
    assert r.status_code == 200
    data = loads(r.content.decode('gbk'))
    wb = Workbook()
    ws = wb.active
    ws.append('开课院系 | 教师名 | 课程号 | 课程名 | 分数1 | 分数2 | 分数3 | 分数4 | 分数5 | 分数6 | 分数7'.split(' | '))
